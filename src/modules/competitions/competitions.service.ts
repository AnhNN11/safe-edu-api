...
export class CompetitionsService {
  ...
  async findAll(searchPhase?: string, page?: number, limit?: number, sortBy?: string, sortOrder?: 'asc' | 'desc') {
    return await this.competition_repository.findAll(searchPhase, Number(page), Number(limit), sortBy, sortOrder);
  }
  ...
  async remove(id: string | Types.ObjectId): Promise<Competition | null>  {
    try {
      ...
      throw new BadRequestException({
        status: HttpStatus.BAD_REQUEST,
        message: "Đã có lỗi xảy ra trong lúc xóa cuộc thi, vui lòng thử lại sau",
        details: `Đã có lỗi xảy ra: ${error.message}`
      })
    }
  }
  ...
  async findBySlug(slug: string): Promise<Competition> {
    try {
      return this.competition_repository.findBySlug(slug);
    } catch (error) {
      throw new BadRequestException({
        status: HttpStatus.BAD_REQUEST,
        message: "Đã có lỗi xảy ra trong lúc tìm kiếm mã định danh, vui lòng thử lại sau",
        details: `Đã có lỗi xảy ra: ${error.message}`
      })
    }
  }
